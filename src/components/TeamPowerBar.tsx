...
export default function TeamPowerBar({ rateLimitPercent }: TeamPowerBarProps) {
  const isUnknown = rateLimitPercent < 0;
  const power = isUnknown ? 0 : Math.max(0, Math.min(100, 100 - rateLimitPercent));

  const barColor = isUnknown ? "#9ca3af" : power > 60 ? "#4ade80" : power > 30 ? "#facc15" : "#ef4444";
